...
const QuizPage: React.FC<QuizPageProps> = ({params}) => {
    ...
    const saveLastChapterAcessedId = useCallback(
        async ()=>{
            try {
                const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/user/save-last-chapter-accessed`,{
                    method: 'PUT',
    ...
    const fetchMaxChapters = useCallback(
        async () => {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/chapter/get-max-number-chapters/${userId}/${id}`,{
                headers:{
                    'Authorization': `Bearer ${token}`, //cabeçalho de autorização
                },
            });
    ...
    const updateUserPoints = async (userId: number, points: number) => {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/user/update-user-points`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`, //cabeçalho de autorização
